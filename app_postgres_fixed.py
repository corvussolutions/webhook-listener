...
FIXED VERSION: Handles missing processing_notes column gracefully
"""
...
def init_database():
    ...
    try:
        ...
        # Check if processing_notes column exists, add if missing
        cursor.execute("""
            SELECT column_name 
...
def index():
    ...
    try:
        ...
        return jsonify({
            'status': 'healthy',
            'service': 'LinkedIn Webhook Listener (PostgreSQL) - FIXED',
            'total_contacts': contact_count,
            'webhooks_last_24h': recent_webhooks,
            'database': 'connected',
            'version': 'fixed_schema'
        })
...
def webhook_logs():
    """View recent webhook logs - FIXED to handle missing column gracefully"""
    try:
        ...
        # Try with processing_notes first, fall back without it
        try:
            cursor.execute("""
...
@app.route('/fix-schema', methods=['POST'])
def fix_schema():
    """Manual schema fix endpoint - adds missing processing_notes column"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        if cursor.fetchone():
            return jsonify({
                'status': 'success',
                'message': 'processing_notes column already exists'
            })
        ...
        return jsonify({
            'status': 'success',
            'message': 'Successfully added processing_notes column'
        })
